...
int8_t yellowButtonCounter = 0;
boolean yellowPressed = false;

int8_t greenButtonCounter = 0;
boolean greenPressed = false;

int8_t blueButtonCounter = 0;
boolean bluePressed = false;

int8_t debouse = 32;

unsigned long globalCounter = 0;
unsigned long deltaCounter = 0;
unsigned long priorCounter = 0;
...
void setup() {
  ...
  PCMSK2 = _BV(PCINT20);            // Enables RD4 as interrupts
...
void loop() {
  if (globalCounter != priorCounter) {
    IncrementButtonCounters();
    DoActions();
  }
  refresh();
}
...
void IncrementButtonCounters() {
  deltaCounter = globalCounter - priorCounter;

  if (isGreenRunning) {
    greenCounter += deltaCounter;
  }
  if (greenPressed) {
    greenButtonCounter += deltaCounter;
  }
  if (yellowPressed) {
    yellowButtonCounter += deltaCounter;
  }
  if (bluePressed) {
    blueButtonCounter += deltaCounter;
  }

  priorCounter = globalCounter;
}

void DoActions() {
  if (greenButtonCounter >= debouse) {
  ...
  if (yellowButtonCounter >= debouse) {
    ...
    yellowButtonCounter = 0;
    yellowPressed = false;
  }

  if (blueButtonCounter >= debouse) {
